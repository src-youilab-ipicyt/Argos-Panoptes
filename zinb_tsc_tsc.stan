...
model {
  ...
  psi[1] ~ normal(0,sigma_psi1); // weakly informative prior
  psi_k[1] ~ normal(0,sigma_psi1); // weakly informative prior
  
  mu_[1]   ~ normal(0,sigma_mu1_); // weakly informative prior
  beta_[1] ~ normal(0,sigma_beta1_); // weakly informative prior
  
  psi_[1] ~ normal(0,sigma_psi1_); // weakly informative prior
  psi_k_[1] ~ normal(0,sigma_psi1_); // weakly informative prior
  
  for (n in 2:T) {
    beta[n] ~ normal(beta[n-1], sigma_zeta);
    mu[n]   ~ normal(mu[n-1] + beta[n-1], sigma_eta); 
    
    psi[n]   ~ normal(rho*(psi[n-1]*cos(lambda_c) + psi_k[n-1] * sin(lambda_c)), sigma_kappa);
    psi_k[n] ~ normal(rho*(-psi[n-1]*sin(lambda_c) + psi_k[n-1]*cos(lambda_c)), sigma_kappa);  
    
    beta_[n] ~ normal(beta_[n-1], sigma_zeta_);
    mu_[n]   ~ normal(mu_[n-1] + beta_[n-1], sigma_eta_); 
    
    psi_[n]   ~ normal(rho_*(psi_[n-1]*cos(lambda_c_) + psi_k_[n-1] * sin(lambda_c_)), sigma_kappa_);
    psi_k_[n] ~ normal(rho_*(-psi[n-1]*sin(lambda_c_) + psi_k_[n-1] * cos(lambda_c_)), sigma_kappa_);        
  }
  ...
  for (n in 1:T) {
      ...
      trend  = mu[n]  + X[n,1:S]  * gammas  + psi[n];
      trend_ = exp(mu_[n] + X_[n,1:S_] * gammas_ + psi_[n]);  
